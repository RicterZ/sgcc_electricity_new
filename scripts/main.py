...
def main():
    ...
    updater = SensorUpdater(HASS_URL, HASS_TOKEN)
    ...
    schedule.every().day.at(JOB_START_TIME).do(run_task, fetcher, updater)
    schedule.every().day.at(next_run_time.strftime("%H:%M")).do(run_task, fetcher, updater)
    run_task(fetcher, updater)
